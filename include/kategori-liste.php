				...
				if($veriler!=false)
				{
					...
					for($i=0;$i<count($veriler);$i++)
					{
                      ...
                      <label class="custom-control-label" for="customSwitch3<?=$veriler[$i]["ID"]?>"></label>
                          ...
                          <td>
                          <a href="<?=SITE?>kategori-duzenle/<?=$veriler[$i]["ID"]?>" class="btn btn-warning btn-sm"><i class="fas fa-pen"></i> Düzenle</a>
                          <?php
                            if($veriler[$i]["tablo"]=="modul")
                            {
                            ...
                            {
                              ?>
                              <a href="<?=SITE?>kategori-sil/<?=$veriler[$i]["ID"]?>" class="btn btn-danger btn-sm silmeAlani"><i class="fas fa-times-circle"></i> Kaldır</a>
                              <?php
                            }
                          ?>
